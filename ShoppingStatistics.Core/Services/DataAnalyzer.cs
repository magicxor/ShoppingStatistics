...
using ShoppingStatistics.Core.Models;

namespace ShoppingStatistics.Core.Services
{
    public class DataAnalyzer
    {
        ...
        public IList<StatisticsItem<TKey, long?>> GetStatistics<TModel, TKey>(
            IEnumerable<TModel> data,
            Func<TModel, TKey> keySelector,
            Func<TModel, long?> valueSelector)
        {
            Func<IList<long?>, double> averageSelector = (values) => { return values.Where(v => v.HasValue).Average(v => v.Value); };
            Func<IList<long?>, double> medianSelector = (values) => { return values.Where(v => v.HasValue).Select(v => v.Value).Median(); };
            Func<IList<long?>, long?> sumSelector = (values) => { return values.Where(v => v.HasValue).Sum(v => v.Value); };
            return GetStatistics(data, keySelector, valueSelector, averageSelector, medianSelector, sumSelector);
        }
